...
if (isset($_POST['name']) && isset($_POST['pass']))
 {
    ...
    function validate($data){
     ...
     if (empty($name)) 
     {
        header("Location: admin_home.php?error= Admin Name is required");
        exit();
     }
     elseif (empty($pass)) 
     {
        header("Location: admin_home.php?error= Password is required");
        exit();
     }
     ...
     else
      {
        ...
        if(mysqli_num_rows($result)==1)
        {
            ...
            // signal the user there was an error entering the information
                        ...
                        header("Location: admin_home.php?error=Incorrect user name or password!");
                        exit();
                  }        
         }
          // signal the user there was an error entering the information
